...
# ── Config ───────────────────────────────────────────────────────────────────
MODEL_BASE="model_hub/Mistral-7B-v0.1"
...
run_divergence() {
    ...
    local DATA_BASE="data/Ultrachat200k/fSWIFT_mis7b_${DIV}"
    ...
    for ITE in 0 1 2 3; do
        ...
        if [ "$ITE" -eq 0 ]; then
            DATASETS="[\"Ultrachat200k/fSWIFT_mis7b_${DIV}/ite0\"]"
        else
            PREV_ITE=$((ITE - 1))
            DATASETS="[\"Ultrachat200k/fSWIFT_mis7b_${DIV}/ite${PREV_ITE}\",\"Ultrachat200k/fSWIFT_mis7b_${DIV}/ite${ITE}\"]"
        fi
        ...
        if [ ! -f "${CKPT_BASE}/ite${ITE}/model.safetensors" ] && \
           [ ! -f "${CKPT_BASE}/ite${ITE}/model-00001-of-00002.safetensors" ]; then
            LATEST_OUT=$(find "${REPO_ROOT}/output" -maxdepth 6 -type d -name "ite${ITE}_*" \
                         -newer "${REPO_ROOT}/scripts/run_divergences_qwen32b_mis7b.sh" \
